...
#include <algorithm> // Para std::upper_bound, std::find
#include <fstream>   // Para std::ofstream
#include <cstdlib>   // Para system()

// Implementación del constructor de NodoBPlus
bPlussTree::NodoBPlus::NodoBPlus(bool hoja) : esHoja(hoja), siguiente(nullptr) {}

// Implementación del constructor de bPlussTree
bPlussTree::bPlussTree(int max) : maxClavesHoja(max)
{
    raiz = new NodoBPlus(true);
    minClavesHoja = (maxClavesHoja + 1) / 2;
    minClavesInterno = (maxClavesHoja + 1) / 2 - 1;
    minHijosInterno = (maxClavesHoja + 1) / 2;
}
...
bool bPlussTree::manejarSubflujoHoja(NodoBPlus *nodo, NodoBPlus *padre, int indiceEnPadre)
{
    if (indiceEnPadre > 0)
...
bool bPlussTree::manejarSubflujoInterno(NodoBPlus *nodo, NodoBPlus *padre, int indiceEnPadre)
{
    if (indiceEnPadre > 0)
    { // Intento de redistribución con hermano izquierdo
        NodoBPlus *hermanoIzq = padre->hijos[indiceEnPadre - 1];
...
void bPlussTree::actualizarSeparadoras(NodoBPlus *padre)
{
...
bool bPlussTree::eliminarRec(NodoBPlus *nodoActual, int clave, NodoBPlus *padre, int indiceEnPadre)
{
...
void bPlussTree::generarImagen()
{
    ...
    system("start arbol.png"); // o "xdg-open arbol.png" en Linux, "open arbol.png" en macOS
